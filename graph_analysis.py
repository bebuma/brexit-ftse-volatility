...
class DataProcessor:
    ...
    def get_raw_data(self, file_name):
        ...
        try:
            ...
            if "Date" in df.columns:
                ...
                df["Date"] = pd.to_datetime(df['Month'], format='%Y-%m').add(pd.offsets.MonthEnd(0))
                del df["Month"]
            else:
                print("Please change raw data file to .csv or .xlsx")
                return None
    ...
    def get_multi_data(self, file_name):
        ...
        for y in range(2016, 2022):
            new_file_name = file_name.split(".")[0] + "-" + str(y) + "." + file_name.split(".")[1]
            df = self.get_raw_data(new_file_name)
            df_all.append(df)
        final_df = pd.concat(df_all)
        final_df = final_df.sort_values(by='Date').reset_index(drop=True)
        final_df["Close"] = final_df["Close"].str.split("%", n=1, expand=True)[0].astype(float)
        return final_df
    ...
    def filter_dataframe(self, df, y_start, y_end=""):
        ...
        filter_df = df.loc[(df['Date'] >= f'{y_start}-01-01') & (df['Date'] <= f'{y_end}-12-31')]
        return filter_df
...
class GraphGenerator:
    ...
    def generate_graph(self, line_input, df_announce, line_name, col_name, y="all"):
        ...
        plt.savefig(self.output_folder_path + line_name + "_" + col_name.lower() + "_" + str(y), bbox_inches='tight')
        plt.clf()
        plt.close()

    def generate_multi_graph_multi_line(self, line_input_1, line_input_2, df_announce, line_name, col_name, y="all"):
        ...
        fig.savefig(self.output_folder_path + "libor_vol" + "_" + str(y), bbox_inches='tight')
        fig.clear(True)
        plt.close()
    ...
    def generate_multi_graph(self, df, df_vol, df_announce, line_name, y):
        ...
        fig.savefig(self.output_folder_path + line_name + "_" + str(y), bbox_inches='tight')
        fig.clear(True)
        plt.close()
        del fig
...
def main():
    ...
    for y in range(2016, 2021):
        ...
        for line_name in ["ftse", "s&p", "euro"]:
            ...
            graph_generator.generate_multi_graph(filter_df, filter_df_vol, filter_annouce, line_name, y)
        ...
        graph_generator.generate_multi_graph_multi_line(libor_dict, libor_dict_vol, filter_annouce, "libor_vol", "vol", y)
    ...
    graph_generator.generate_multi_graph(filter_reer, filter_reer_vol, filter_annouce, "reer_vol", "all")
    print(f"Finished generating REER")

    filter_google = data_processor.filter_dataframe(google, 2016, 2021)
    graph_generator.generate_graph(filter_google, filter_annouce, "google", "Value", "all")
